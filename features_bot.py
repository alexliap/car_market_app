...
if os.path.isfile('/Users/alexanderliapatis/Desktop/Projects/msc_ml_project/RawData/features_data.csv'):
    ft_data = pd.read_csv('/Users/alexanderliapatis/Desktop/Projects/msc_ml_project/RawData/features_data.csv')
else:
...
for vin in vin_list:
    try:
        ...
        if vin not in ft_data['vin'].values:
            ...
            for item in response.json().keys():
                if item not in['options', 'years', 'colors']:
                    ...
                    if isinstance(response.json()[item], dict):
                        data = pd.concat([data, pd.json_normalize(response.json()[item]).add_suffix('_' + item)], axis = 1)
                    else:
            ...
            ft_data.to_csv('/Users/alexanderliapatis/Desktop/Projects/msc_ml_project/RawData/features_data.csv', index = False)
    except:
        pass
